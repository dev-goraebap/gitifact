...
const path = '.tryce/spec/example/requirements.md';
const req = (id = 'R-v4n6paz2ce', title = '게시물 등록', body = '조건: 등록 요청\n기대 동작: 저장') => `## ${title}\n<!-- tryce-req: ${id} -->\n\n${body}\n`;
const doc = (body = req(), id = 'S-k7m2xqab3d', title = '게시물') => `<!-- tryce-spec: ${id} -->\n# ${title}\n\n${body}`;
...
test('preview rejects malformed, ambiguous and duplicate identities', () => {
  for (const s of [doc().replace('S-k7m2xqab3d', 'S-bad'), doc().replace('<!-- tryce-req:', '<!-- missing:'), doc(req()+req()), doc(req()+'\n```\n'), doc().replace('## 게시물 등록','  ## 게시물 등록'), doc().replace('## 게시물 등록','제목\n---')]) assert.throws(()=>parseSpecPreview(path,s));
  assert.throws(()=>validatePreviewSnapshot([parse(),parse(undefined,'S-u2gk5wd7ra','.tryce/spec/other/requirements.md')]));
});
test('create, delete, edit, reverted edit and title/path-only spec changes', () => {
  ...
  const renamed=parseSpecPreview('.tryce/spec/renamed/requirements.md',doc(req(),undefined,'새 이름'));
...
test('move and move+edit are not delete/create; reasons attach by new history identity', () => {
  const before=[parse(),parse('', 'S-u2gk5wd7ra','.tryce/spec/other/requirements.md')];
  const history=JSON.stringify({id:'H-b6fq2ry4ns',requirements:['R-v4n6paz2ce'],reason:'잘못된 소속 정정'});
  const after=[parse(''),parseSpecPreview('.tryce/spec/other/requirements.md',doc(req(),'S-u2gk5wd7ra'),history)];
  ...
  after[1]=parseSpecPreview('.tryce/spec/other/requirements.md',doc(req(undefined,'새 제목'),'S-u2gk5wd7ra'),history);
