...
import { editSpecPreview, parseSpecPreview, renderSpecPreview, emptyWiki } from '../dist/index.js';
const bundle=(...specs)=>({specs,wiki:emptyWiki()});
const s='S-k7m2xqab3d',r='R-v4n6paz2ce';
const source=()=>parseSpecPreview('.gitifact/spec/posts/requirements.md',`---\nid: ${s}\n---\n# 게시물\n\n## 등록\n<!-- gitifact-req: ${r} -->\n\n저장합니다.\n`);
test('draft edits preserve IDs, reject injected structure and leave inputs untouched',()=>{
  ...
  const edited=editSpecPreview(original,[{type:'update',id:r,title:'게시물 작성',body:'제목과 본문을 저장합니다.'},{type:'rename-spec',id:s,title:'게시물 관리'}],()=>assert.fail('must not allocate'));
...
test('allocation retries collisions and moving keeps identity and exact parsed body',()=>{
  ...
  const edited=editSpecPreview(original,[{type:'create',feature:'profile',title:'프로필'},{type:'move',id:r,feature:'profile'}],()=>++attempts===1?s:'S-u2gk5wd7ra');
  ...
  assert.ok(renderSpecPreview(edited.specs[1]).startsWith('---\nid: S-u2gk5wd7ra\n---\n\n# 프로필\n\n## 등록\n<!-- gitifact-req: '+r+' -->\n'));
  // A spec written by 0.4.x opens with a marker line; it reads the same and renders back as frontmatter.
  const legacy=parseSpecPreview('.gitifact/spec/posts/requirements.md',`<!-- gitifact-spec: ${s} -->\n# 게시물\n\n## 등록\n<!-- gitifact-req: ${r} -->\n\n저장합니다.\n`);
  assert.deepEqual(legacy,source());assert.equal(renderSpecPreview(legacy),renderSpecPreview(source()));
...
test('unknown fields, invalid paths, missing targets and duplicate operations fail',()=>{
  for(const op of [{type:'create',feature:'../escape',title:'bad'},{type:'create',feature:'con',title:'bad'},
    ...
    {type:'move',id:r,feature:'posts'}]) assert.throws(()=>editSpecPreview(bundle(source()),[op],()=>s));
});
