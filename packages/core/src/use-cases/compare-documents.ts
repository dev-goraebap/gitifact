...
interface Side { docs: Map<string, { doc: Doc; text: string }>; reasons: Map<string, DocReason> }
...
function sideOf(files: ReadonlyMap<string, string>): Side {
  const side: Side = { docs: new Map(), reasons: new Map() };
  for (const [path, source] of files) {
    try {
      const where = classifyDocPath(path);
      if (where.type === 'reasons') { for (const r of parseReasonLines(path, source)) side.reasons.set(r.id, r); continue; }
 ...
 */
export function compareDocumentSets(before: ReadonlyMap<string, string>, after: ReadonlyMap<string, string>): { changes: DocChange[]; reasons: DocReason[] } {
  ...
  return { changes, reasons: [...to.reasons.values()].filter(r => !from.reasons.has(r.id)) };
}
