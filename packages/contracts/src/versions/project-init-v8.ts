...
// being converted; v8 adds `lineEndings`, the .gitifact/.gitattributes init writes. Earlier versions had no consumers
// outside the CLI and were removed.
export const projectInitV8 = z.discriminatedUnion('ok', [
  z.strictObject({ contract: z.literal('project-init'), version: z.literal(8), ok: z.literal(true),
    ...
    // The file that keeps documents in LF on every OS. `created` is false when it was already there or the run is planned.
    lineEndings: z.strictObject({ path: z.literal('.gitifact/.gitattributes'), created: z.boolean() }),
  ...
  z.strictObject({ contract: z.literal('project-init'), version: z.literal(8), ok: z.literal(false), error: z.strictObject({ code: z.string(), message: z.string() }) }),
]);
export type ProjectInitV8 = z.infer<typeof projectInitV8>;
