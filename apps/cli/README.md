...
Install or update with `npm install -g gitifact@latest`. Run `gitifact init`, then read the GITIFACT block it writes into the project's agent instruction file and follow it in the current session. `init` also asks npm for a newer release and reports it in `update` and `install`, so an older install is noticed; the check never blocks initialization and `GITIFACT_NO_UPDATE_CHECK=1` turns it off.
