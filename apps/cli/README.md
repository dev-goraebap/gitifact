...
`gitifact docs` lists the guidance topics (`workflow`, `spec`, `design`, `wiki`, `commit`) and `gitifact docs <topic>` prints one: the format part the CLI enforces, followed by operating guidance. A project replaces that guidance by running `gitifact docs <topic> --eject`, which copies the bundled text to `.gitifact/overrides/<topic>.md`; edit it freely, delete it to return to the bundled text. The block summarises the rules and points agents at these topics for the exact input formats.

Records are Markdown with a YAML frontmatter `id` (`S-…` for a feature, `W-…` for a wiki page); requirement sections keep their `<!-- gitifact-req: R-… -->` line. `.gitifact/wiki/` holds the project wiki (entry page `README.md`, free folders), `.gitifact/assets/` holds files the documents reference (any type, no IDs), and `spec working` reports advisory warnings for dangling relative links, assets over 1 MB or outside the recommended extensions, and assets nothing references. Design files may list the documents they drew on under `sources` in the frontmatter.
