...
import type { ProjectConfig } from '@tryce/core';
...
export function initRepository(cwd: string, inherited: NodeJS.ProcessEnv = process.env) {
  ...
  return {
    async inspect() {
      const state = await createRepositoryReader(cwd, { env }).read();
      const root = state.repository.rootPath;
      const gitDir = decodeGitLine(await git(['rev-parse', '--absolute-git-dir'], root));
      ...
      const indexPath = decodeGitLine(await git(['rev-parse', '--path-format=absolute', '--git-path', 'index'], root));
      const index = await readFile(indexPath).catch(error => { if (error.code === 'ENOENT') return Buffer.alloc(0); throw error; });
      const staged = await git(['ls-files', '--stage', '-z', '--', '.tryce/config.json'], root);
      const committed = state.head.commit ? await git(['ls-tree', '-z', state.head.commit, '--', '.tryce/config.json'], root) : Buffer.alloc(0);
