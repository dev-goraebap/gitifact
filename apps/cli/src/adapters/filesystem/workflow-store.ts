...
import { fileInfo, readConfigFile } from './config-file.js';
...
export async function readRequirementSets(root: string) {
  const dir = join(root, 'specs'); const sets: RequirementSet[] = []; const contents = new Map<string, string>();
  if (!await regularDirectory(dir)) return { sets, contents, stamp: digest('[]') };
  const check = await guard(dir); const names = (await readdir(dir)).sort();
  if (names.length > 1000) throw new InitError('REQUIREMENT_LIMIT', 'specs 항목 수가 1,000개를 초과합니다.');
  let bytes = 0;
  for (const name of names) {
    if (!specPattern.test(name)) continue;
    const parent = join(dir, name); const info = await fileInfo(parent);
    if (info?.isSymbolicLink()) throw new InitError('PATH_CONFLICT', 'specs의 링크를 따라가지 않습니다: ' + name);
    if (!info?.isDirectory()) continue;
    const verify = await guard(parent); const path = `specs/${name}/tryce.json`;
  ...
  await check(); if (JSON.stringify(names) !== JSON.stringify((await readdir(dir)).sort())) throw new InitError('INPUT_CHANGED', 'specs 목록이 변경됐습니다.');
...
export async function workflowTransaction<T>(cwd: string, write: boolean, action: (context: WorkflowContext) => Promise<T>, env = process.env) {
  ...
  async function context() {
    ...
    for (const prefix of ['.tryce/notes/', 'specs/']) {
      const paths = await repo.trackedPaths(root, prefix, first.state.head.commit);
      const missing = prefix === 'specs/' ? paths.filter(p => /^specs\/[^/]+\/tryce\.json$/.test(p) && !records.contents.has(p)) : paths.filter(p => !notes.paths.includes(p));
    ...
    for (const set of records.sets) {
      for (const text of await repo.recordVersions(root, `specs/${set.spec}/tryce.json`, first.state.head.commit)) {
...
export async function saveRequirementSet(root: string, set: RequirementSet, recheck: () => Promise<void>, checkIgnore: (path: string) => Promise<void>, exists: boolean) {
  ...
  if (!await fileInfo(join(root, 'specs', set.spec)) && await regularDirectory(join(root, 'specs')) && (await readdir(join(root, 'specs'))).length >= 1000) throw new InitError('REQUIREMENT_LIMIT', '새 spec 디렉터리를 추가할 수 없습니다.');
  for (const part of ['specs', `specs/${set.spec}`]) {
  ...
  const path = `specs/${set.spec}/tryce.json`;
