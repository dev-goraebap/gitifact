---
title: 문서 문체
description: 위키·요구사항·설계에 공통으로 적용하는 문체, 본문과 변경 이유의 구분, 인용·Alert·다이어그램, AI 슬롭 제거
---
...
본문에는 지금 유효한 동작과 규칙만 쓴다. '전에는 …했는데 …라서 바꿨다', 바꾼 날짜와 계기, 옛 방식, 측정해 가며 고른 과정은 본문에 두지 않고 커밋의 변경 이유에 쓴다(`gitifact guide show commit`). 읽는 사람은 이유를 `gitifact docs history <ID>`로, 옛 원문을 `gitifact docs show <ID> --ref <커밋>`으로 본다.

문서를 고칠 때는 바뀐 문장을 고쳐 쓰고 옛 동작을 설명하는 문장을 덧붙이지 않는다. 되돌리면 안 되는 결정과 기각한 안은 경위가 아니라 지금의 제약이다. 설계에서는 `overview.md`의 결정 표에 한 줄씩 두고(`gitifact guide show design`), 여러 기능에 걸치면 위키의 운영 방침이 정한 곳에 둔다. 결정 기록(ADR)처럼 맥락과 대안 자체가 본문인 문서는 그 형식을 따른다.
