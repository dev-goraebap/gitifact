---
title: 문서 문체
description: 지침·요구사항·설계에 공통으로 적용하는 문체, 본문과 변경 이유의 구분, 인용·Alert·다이어그램, AI 슬롭 제거
---
...
문서를 고칠 때는 바뀐 문장을 고쳐 쓰고 옛 동작을 설명하는 문장을 덧붙이지 않는다. 되돌리면 안 되는 결정과 기각한 안은 경위가 아니라 지금의 제약이다. 설계에서는 `overview.md`의 결정 표에 한 줄씩 두고(`gitifact guide show design`), 여러 기능에 걸치면 그 영역 지침의 결정 표에 둔다(`gitifact guide show instructions`).
