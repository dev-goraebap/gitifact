...
| 파일 | 다루는 것 |
| :--- | :--- |
| `overview.md` (필수) | 범위와 접근 방식, 구성 요소와 경계, 결정 표, 미결 사항 |
...
본문의 문체는 `gitifact guide show writing`을 따른다. 본문에는 지금의 구조와 동작을 쓰고, 바뀐 경위·날짜·옛 방식은 커밋의 변경 이유에 쓴다. 확정한 것, 구현에서 관측한 것, 제안을 구분한다. 결정 목록만으로 구현 설명을 대신하지 않는다.

되돌리기 어려운 결정과 기각한 대안은 `overview.md`의 `## 결정` 절에 표로 한 줄씩 둔다. 다른 축 파일에는 결정의 이유를 다시 쓰지 않는다.
...
| 결정 | 이유 | 기각한 안 |
| :--- | :--- | :--- |
| 첨부 파일은 게시물 삭제 뒤 정리 작업이 지운다 | 삭제 요청이 파일 수에 따라 느려지지 않는다 | 요청 안에서 함께 삭제(첨부가 많으면 시간 초과) |
```
...
개정 전에 그 기능의 설계 파일들과 관련 요구사항을 읽고 영향을 받는 파일만 고친다. 바뀐 문장은 고쳐 쓰고 '전에는 …했다'는 문장을 덧붙이지 않는다. 바뀐 경위는 변경 이유에 쓰고 과거 원문은 Git이 보존한다. 결정이 바뀌면 결정 표의 그 줄을 고친다. 요구사항을 바꾸면 그것을 가리키는 설계도 검토하고(`gitifact docs show <R-ID>`의 "가리키는 문서"), 설계만 바뀌면 요구사항을 억지로 고치지 않는다.
