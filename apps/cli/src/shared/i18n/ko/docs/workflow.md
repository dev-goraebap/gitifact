...
현재 경로·브랜치·Git 상태와 기존 staging을 확인하고 적용되는 AGENTS.md·CLAUDE.md를 원문으로 읽는다. 프로젝트가 정한 CLI 실행 방법을 우선하고, 별도 지정이 없으면 전역 `gitifact`를 쓴다. 아래 `gitifact`는 선택한 실행 방법을 뜻한다. 전역 설치본은 PC에 한 버전뿐이며, 그 버전이 프로젝트 기준과 다르면 명령마다 안내가 나온다.

설치와 업데이트는 에이전트가 한다. 사용자에게 명령을 건네 직접 설치하게 하지 않는다. 명령이 없으면 `.gitifact/config.json`의 `cli`(프로젝트가 기준으로 삼는 CLI 버전)로 `npm install -g gitifact@<버전>`을, 업데이트는 안내의 설치 명령을 그대로 보여 주며 실행해도 되는지 묻는다. 질문에 정확한 명령이 들어 있어야 사용자의 짧은 허용이 그 명령에 대한 허용으로 읽힌다. 허용하면 직접 실행한다. 에이전트 도구가 실행을 막으면 그 도구의 방식으로 승인을 요청한다. 샌드박스 밖 실행을 요청하면 승인 창이 뜨는 도구는 그 요청을 하고, 설정으로만 허용되는 도구는 어느 설정에 무엇을 더하면 되는지 알린다. Windows PowerShell에서 스크립트 실행이 막혀 있으면(`npm.ps1`을 로드할 수 없음) `npm.cmd`·`gitifact.cmd`로 실행한다. 설치가 끝나기 전에도 가능한 조사는 계속하되, CLI 실행 없이 문서 ID 발급·검사·커밋을 대신하거나 완료했다고 보고하지 않는다.

모든 명령은 결과보다 먼저 stderr에 버전 안내를 한 줄 낼 수 있다. JSON 출력은 바뀌지 않는다. 프로젝트 기준이 실행 중인 CLI보다 새 버전이면 그 버전의 설치를, 새 버전이 나왔으면 설치 후 `gitifact update`를, 실행 중인 CLI가 프로젝트 기준보다 새 버전이면 `gitifact update`로 기준을 올리라고 알린다. 한 번에 하나만 이 순서로 낸다. 새 버전 확인은 사용자 캐시만 읽고, 마지막 확인이 1시간을 넘으면 명령과 따로 뒤에서 레지스트리를 조회해 캐시를 갱신한다. 안내가 보이면 다음 작업 전에 설치 명령을 보여 주며 지금 업데이트(권장)와 나중에 중 하나를 사용자에게 묻는다. 동의 전에는 설치하거나 지침을 갱신하지 않는다. 나중에를 고르면 `gitifact update --later`를 실행한다. 그 버전의 안내는 24시간 나오지 않고, 더 새 버전이 나오면 바로 다시 나온다. `GITIFACT_NO_UPDATE_CHECK`는 새 버전 조회만 끈다. 프로젝트 기준 비교는 로컬 파일만 읽으므로 계속한다.

업데이트는 에이전트가 `npm install -g gitifact@<새 버전>`으로 설치한 뒤 `gitifact update`를 실행한다. 프로젝트 의존성으로 관리하면 해당 패키지 관리자로 버전을 올리고 업데이트한 설치본을 실행한다.
