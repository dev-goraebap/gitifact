...
- **커밋된 기록:** 이미 커밋된 기록을 고치거나 지우면 `changes list`와 `docs check`가 알리고 `changes commit`이 거부한다. HEAD대로 되돌리고 바뀐 결정은 새 기록으로 쓴다.
- **검사:** 커밋 직전에 `docs check`와 같은 검사를 돌린다. 문제가 있거나 문서·커밋할 기록에 `draft: true`가 남아 있으면 커밋하지 않는다. 링크·에셋 경고는 커밋을 막지 않는다.
