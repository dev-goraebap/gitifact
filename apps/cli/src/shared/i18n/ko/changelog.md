## 0.8.4 - 2026-09-27
### Changed
- 에이전트가 Gitifact 설치와 업데이트를 사용자에게 넘기지 않습니다. 설치 명령을 그대로 보여 주며 실행해도 되는지 묻고, 허용하면 직접 설치합니다. Claude Code 자동 모드에서도 명령이 담긴 질문에 "네"라고만 답하면 진행됩니다. 도구가 그래도 막으면 에이전트가 그 도구의 승인 창이나 권한 설정을 안내합니다. `update`로 지침 블록을 갱신하세요.
- Windows PowerShell에서 스크립트 실행이 막혀 `npm.ps1`을 로드할 수 없으면 에이전트가 `npm.cmd`·`gitifact.cmd`로 실행합니다. 시작하기에도 같은 방법을 적었습니다.
### Fixed
- `init`이 프로젝트 기준을 스스로 올리는데도 그 앞에 "gitifact update로 프로젝트 기준을 올릴 수 있습니다" 안내를 보이던 문제를 고쳤습니다.

## 0.8.3 - 2026-09-26
### Added
- 모든 명령이 결과 앞에 버전 안내를 한 줄 보입니다. 새 버전이 나왔을 때, 프로젝트가 이 CLI보다 새 버전 기준일 때, 이 CLI가 프로젝트 기준보다 새 버전일 때 알립니다. 새 버전은 사용자 캐시에서 읽고 1시간마다 명령과 따로 뒤에서 확인하므로 명령은 네트워크를 기다리지 않습니다. `GITIFACT_NO_UPDATE_CHECK`는 새 버전 확인만 끕니다.
- `update --later`로 안내된 새 버전을 24시간 동안 쉽니다. 그보다 새 버전이 나오면 바로 안내합니다.
- `.gitifact/config.json`에 프로젝트 기준 버전(`cli`)과 지침 블록 언어(`language`)를 적습니다. `init`·`update`가 쓰고 기준 버전은 낮추지 않습니다. 설정에 이 CLI가 모르는 항목이 있어도 멈추지 않습니다.
### Changed
- 0.8.3 이상이 필요합니다. `cli`가 들어간 설정은 0.8.2 이하에서 `INVALID_CONFIG`로 멈추므로, 팀원 모두 올린 뒤 `update`하세요.
- 설치 안내를 전역 설치(`npm install -g gitifact@<버전>` 뒤 `gitifact update`)로 바꿨습니다. 지침 블록·README·시작하기가 npx 대신 전역 명령을 안내합니다.
- 지침 블록의 첫 줄(버전·언어·schemaVersion)을 없앴습니다. 세션을 시작할 때 버전을 확인하고 `update --check`를 실행하던 단계 대신, 명령 앞의 버전 안내를 보면 지금 업데이트할지 나중에 할지 묻습니다. 블록 문구가 바뀌지 않는 배포에서는 블록도 바뀌지 않습니다.
- 블록 언어는 설정의 `language`를 따르고 `--lang`을 줄 때만 바뀝니다. 0.8.2 이하에서 만든 블록은 첫 줄의 언어를 한 번 읽어 설정에 옮깁니다.
- `update --commit`이 블록과 함께 `cli`·`language`만 바뀐 `config.json`을 `chore(gitifact): update project to gitifact v<버전>`으로 커밋합니다. `update`의 JSON은 v6으로 `project`를 싣고 설치 명령은 `install.npmGlobal`만 둡니다.
- 프로젝트 기준보다 오래된 CLI의 `update`와 `init`은 지침 블록을 바꾸지 않습니다.
- 브라우저가 화면마다 따로 있던 로딩 뼈대 대신 공통 로켓 로더를 보입니다. 다른 화면으로 옮길 때는 새 화면이 다 그려질 때까지 지금 화면을 흐리게 두고 그 위에 로더를 보입니다.
- 브라우저 기능 목록에서 제목·설명 글자를 누를 때만 기능이 열리고 행의 빈 곳은 반응하지 않습니다.
### Fixed
- `changes commit`이 `init`이 쓴 `.gitifact/.gitattributes`를 거부해 사용자 프로젝트에서 이 파일을 커밋할 방법이 없던 문제를 고쳤습니다.
