...
1. `gitifact`가 없으면 `.gitifact/config.json`의 `cli` 버전으로 `npm i -g gitifact@<버전>`(Windows PowerShell은 `npm.cmd`)을 실행해도 되는지 명령 그대로 보여 주며 묻고, 허용하면 직접 실행한다. 사용자에게 설치를 떠넘기지 않는다.
...
- 명령 출력 앞에 버전 안내가 보이면 다음 작업 전에 안내의 설치 명령을 그대로 보여 주며 지금 업데이트(권장)할지 나중에(`gitifact update --later`) 할지 묻는다. 허용하면 설치와 `gitifact update`를 직접 실행하고 이 블록을 다시 읽는다. 도구가 막으면 그 도구의 권한 승인을 요청한다.
