...
- **`paths`:** the document files, record files and related code and tests this commit takes. Not every changed document has to be included; the rest stays for a later commit. A moved document needs both its old and new path. For a deleted document, list the deleted path. Include new assets the documents reference (`.gitifact/assets/…`). The `.gitifact/.gitattributes` that `init` writes can be included too. One commit takes up to 128 paths (5,000 for a migration commit with `migration: true`); beyond that, split the commits by decision.
