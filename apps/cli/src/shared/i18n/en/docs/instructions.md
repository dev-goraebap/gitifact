...
Create new instructions with the CLI. It issues an `I-` ID, fills in the frontmatter and adds `draft: true`. Write the body, remove that line and check with `gitifact docs check`.

```text
gitifact docs new instruction code-review --title "Code review" --description "What to check in a review and how to report it. Use when reviewing a change."
```
...
Edit existing instructions directly. To rename one, move the folder and keep the ID; to delete one, delete the folder. If a design's `sources` still names a deleted instruction, fix that design too or `docs check` fails. An instruction folder without `index.md` is an `INSTRUCTION_INDEX_REQUIRED` problem.
...
Write links between instructions and to assets relative to the file, for example `../verification/index.md` or `../../assets/diagrams/flow.png`. `docs check` and `changes list` report a link without a target as a `MISSING_LINK_TARGET` warning.
...
- When a new rule or decision spans features, suggest recording it in an instruction. If the user agrees, edit an existing instruction or create one with `gitifact docs new instruction`, and update the AGENTS.md index.
- The 0.7 wiki (`.gitifact/wiki/`) became instructions in 0.8.0. `docs check` reports any page left there as `WIKI_REMOVED`. Follow `gitifact guide show migrate` to move them.
