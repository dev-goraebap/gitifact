  ...
  try {
    ...
    await previewTransaction(cwd, current.stamp, async () => ({ writes, data: {}, recheck: c.recheck }), rename, async published => {
      ...
      try {
        ...
        for (const [path, value] of started) if (locked.get(path) !== value) fail(t('commit.fileChangedRunning', { path }));
