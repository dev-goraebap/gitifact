...
export const newKinds = ['feature', 'requirement', 'design', 'wiki'] as const satisfies readonly DocKind[];
...
export const runDocsList = (options: Options & { feature?: string; kind?: 'spec' | 'wiki' }) => runCommand('docs', options.format, async () => {
  ...
  let features = options.kind === 'wiki' ? [] : arranged.features;
  ...
  const wiki = options.kind === 'spec' || options.feature !== undefined ? [] : arranged.wiki;
  ...
  const json = {
    ...
    orphans: options.feature === undefined && options.kind !== 'wiki' ? arranged.orphans : [],
    problems,
  };
...
export const runDocsShow = (ids: string[], options: Options & { ref?: string }) => runCommand('docs', options.format, async () => {
  ...
  const shown = await Promise.all([...new Set(ids)].map(async id => {
    ...
    return { id, kind: doc.kind, path: doc.path, title: doc.title, text: await view.read(doc.path), references, referencedBy };
  }));
...
const pathExample: Record<typeof newKinds[number], () => string> = {
  ...
  design: () => t('docs.pathExample.design'), wiki: () => t('docs.pathExample.wiki'),
};
const skeleton: Record<typeof newKinds[number], () => string> = {
  ...
  design: () => t('docs.skeleton.design'), wiki: () => t('docs.skeleton.wiki'),
};

/** Where `docs new <kind> <path>` puts a document: a feature folder, a requirement or design slug in one, or a wiki page. */
function newPath(kind: typeof newKinds[number], path: string): string {
  ...
  const shape = kind === 'feature' ? /^[^/]+$/ : kind === 'wiki' ? /./ : /^[^/]+\/[^/]+$/;
...
export const runDocsNew = (kind: typeof newKinds[number], path: string, options: Options & { title: string; description: string }) => runCommand('docs', options.format, async () => {
  ...
  let id: string; do id = generateId(({ feature: 'S', requirement: 'R', design: 'D', wiki: 'W' } as const)[kind]); while (taken.has(id));
