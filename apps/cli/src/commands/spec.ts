...
import { compareStoreBundles, StoreError, RepositoryReadError, InitError, parseManagedConfig } from '@gitifact/core';
import { storeReader } from '../adapters/git/store-reader.js';
import { withCommandScope } from '../adapters/git/command-scope.js';
import { readWorking, saveWorking } from '../adapters/filesystem/store.js';
import { readFinalChanges } from './store-context.js';
...
export interface SpecControls extends AgentInputControls { stdin?: AsyncIterable<Uint8Array> }
...
export const runSpec = (action: Action, options: Options, controls: SpecControls = {}) => withCommandScope(() => execute(action, options, controls));

async function execute(action: Action, options: Options, controls: SpecControls) {
  const envelope = { contract: 'spec', version: 1 };
  try {
    // Same guards as the full repository inspection, using only the Git reads they need.
    const guard = storeReader(process.cwd()); const { root, gitDir, objectFormat } = await guard.location();
    ...
    if (raw === undefined) throw new StoreError(t('preview.notInitialized'));
    ...
    if (['working', 'save', 'changes', 'commit'].includes(action)) {
      ...
      if (action === 'working') result = narrowWorking(await readWorking({ root, gitDir }), options, await inputs());
      else if (action === 'changes') result = { ...await readFinalChanges(process.cwd()), inputs: await inputs() };
      else {
        const bytes = options.file === '-' ? await readStdin(controls.stdin ?? process.stdin) : await readInputFile(options.file!);
        if (bytes.length > 1024 * 1024) throw new StoreError(t('preview.inputSize'));
        let input: unknown;
        try { input = JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(bytes)); }
        catch { throw new StoreError(t('preview.inputJson')); }
        result = action === 'commit' ? await specCommit(process.cwd(), input, !!options.dryRun)
          : await saveWorking({ root, gitDir }, input);
    ...
    const reader = storeReader(process.cwd());
    if (action === 'read') {
      ...
      process.stdout.write(JSON.stringify({ ...envelope, ok: true, from, to, ...compareStoreBundles(before, after) }) + '\n');
    }
  } catch (error) {
    const known = error instanceof StoreError || error instanceof RepositoryReadError || error instanceof InitError;
...
async function readInputFile(path: string) {
  ...
  if (!source.isFile() || source.size > 1024 * 1024) throw new StoreError(t('preview.inputFile'));
...
async function readStdin(stream: AsyncIterable<Uint8Array>) {
  ...
  for await (const chunk of stream) {
    size += chunk.length;
    if (size > 1024 * 1024) throw new StoreError(t('preview.inputSize'));
    chunks.push(chunk);
  }
  return Buffer.concat(chunks);
}

type Working = Awaited<ReturnType<typeof readWorking>>;
...
function narrowWorking(working: Working, options: Options, inputs: Inputs) {
  ...
  if (options.feature !== undefined) {
    ...
    if (!specs.length) throw new StoreError(t('preview.unknownFeature', { feature: options.feature }));
