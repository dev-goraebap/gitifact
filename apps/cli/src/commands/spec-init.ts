...
export async function initializeSpecProject(cwd: string, dryRun = false, env = process.env, beforePublish?: () => Promise<void>, agentDocs?: AgentDocsOptions) {
  ...
  const checkRecords = async () => {
    ...
    if (entries.some(name => !/^\.init-[a-f0-9-]+\.tmp$/.test(name))) throw new InitError('EXISTING_RECORDS', '설정 없는 .gitifact 자료를 자동 채택하지 않습니다.');
  };
  ...
  const published = async () => {
    if ((await repo.inspect()).stamp !== first.stamp || await readConfigFile(root) !== text) throw new InitError('INPUT_CHANGED_AFTER_WRITE', '설정 생성 후 입력이 변경됐습니다. 현재 파일을 확인하세요.');
  };
  await published();
