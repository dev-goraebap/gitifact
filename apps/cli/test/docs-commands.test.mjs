...
test('docs list shows frontmatter and referenced titles without bodies, narrowed by feature or kind', t => {
  ...
  // Instructions are listed by folder name; init writes no document of its own.
  assert.match(all, /^\[지침\]\n  I-eeeeeeeeee 결제 모듈 \(payment-modules\) — 결제 모듈의 계층\n$/m);
  assert.equal(f.run(['docs', 'list', '--kind', 'instruction']).stdout, `[지침]\n  ${I} 결제 모듈 (payment-modules) — 결제 모듈의 계층\n`);
  const json = f.ok(['docs', 'list', '--feature', 'payment']);
  assert.deepEqual([json.contract, json.version, json.ok, json.instructions, json.problems, 'wiki' in json], ['docs', 1, true, [], [], false]);
  assert.deepEqual(json.features[0].designs[0].requirements, [{ id: R1, title: '결제 취소' }]);
  assert.deepEqual(json.features[0].designs[0].sources, [{ id: I, title: '결제 모듈' }, { title: 'PG사 취소 API', url: 'https://example.test/cancel' }]);
...
test('docs new issues an ID and a draft skeleton that the check refuses until the draft line is gone', t => {
  ...
  const rules = f.run(['docs', 'new', 'instruction', 'naming', '--title', '이름 규칙', '--description', '파일과 변수 이름']);
  ...
  for (const kind of ['wiki', 'note']) assert.notEqual(f.run(['docs', 'new', kind, 'payment/x', '--title', 'a', '--description', 'b']).status, 0, kind);
...
test('docs check reports every problem across files, including references broken by a deletion', t => {
  ...
  assert.match(f.run(['docs', 'check']).stdout, /WIKI_REMOVED .*guide show migrate/);
  const listed = f.ok(['docs', 'list']);
  assert.equal(listed.problems.filter(p => p.code === 'WIKI_REMOVED').length, 1);
  assert.equal(f.run(['docs', 'show', 'W-wwwwwwwwww']).status, 1);
  assert.deepEqual(f.ok(['docs', 'search', '계층']).hits, []);
});
