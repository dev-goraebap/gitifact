...
// In-process checks keep their answer in the test's cache folder, never the user's.
const cacheOnly = { GITIFACT_CACHE_DIR: process.env.GITIFACT_CACHE_DIR };
...
test('check-only reports release states without requiring a project', async () => {
  for (const [latest, status] of [['0.8.0', 'available'], ['0.7.0', 'up-to-date'], ['0.6.0', 'up-to-date'], ['invalid', 'unavailable']]) {
    const dto = updateCheckV1.parse(await checkUpdateCommand('0.7.0', cacheOnly, { fetchLatest: async () => latest }));
    assert.equal(dto.update.status, status);
    assert.equal(dto.command, status === 'available' ? 'npx --yes gitifact@0.8.0 update' : null);
  }
  const offline = await checkUpdateCommand('0.7.0', cacheOnly, { fetchLatest: async () => { throw new Error('offline'); } });
  assert.deepEqual(offline.update, { status: 'unavailable', latestVersion: null });
  const timeout = await checkUpdateCommand('0.7.0', cacheOnly, { fetchLatest: () => new Promise(() => {}), timeoutMs: 20 });
  assert.deepEqual(timeout.update, offline.update);
  let asked = 0;
  const disabled = await checkUpdateCommand('0.7.0', { ...cacheOnly, GITIFACT_NO_UPDATE_CHECK: '1' }, { fetchLatest: async () => { asked++; return '0.8.0'; } });
...
test('check-only preserves old blocks, staged edits, files and HEAD in both languages', async t => {
  ...
  // Only the user's release cache changes: the check keeps its answer there for the notices.
  const files = () => Object.fromEntries(Object.entries(fingerprint(f.root)).filter(([path]) => !path.startsWith('cache/')));
  const before = files();
  for (const cwd of [f.repo, f.root]) for (const language of ['ko', 'en']) for (const format of ['json', 'text']) {
    ...
    else assert.match(result.stdout, /npm install -g gitifact@9\.9\.9\n.*gitifact update/);
    assert.deepEqual(files(), before);
  }
  assert.equal(JSON.parse(readFileSync(join(f.root, 'cache/update.json'), 'utf8')).latest, '9.9.9');
