...
const allPaths=['.gitifact/spec/posts/requirements.md','.gitifact/spec/posts/history.jsonl','app.js','app.test.js'];
function setup(t,format='sha1'){
 ...
 mkdirSync(join(f.repo,'.git/hooks'));f.commit('Adopt gitifact');
...
test('failed pre-commit hook preserves index and allows retry without duplicate commits',t=>{
 ...
 assert.equal(input(f,'commit-apply',p).status,1);assert.equal(existsSync(join(f.repo,'.git/gitifact-spec-commit.lock')),false);assert.equal(existsSync(join(f.repo,'.git/index.lock')),false);
...
test('hook changing the commit leaves recovery evidence and blocks blind retries',t=>{
 ...
 assert.equal(existsSync(join(f.repo,'.git/gitifact-spec-commit.lock/recovery.json')),true);const head=f.git(['rev-parse','HEAD']).stdout;
