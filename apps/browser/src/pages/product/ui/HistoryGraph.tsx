...
export function HistoryGraph({events, selected}: {events:SpecEvent[]; selected?:string | undefined}) {
  ...
  return <svg className={styles.graph} width={HISTORY_GRAPH_WIDTH} height={events.length*HISTORY_ROW_HEIGHT} viewBox={`0 0 ${HISTORY_GRAPH_WIDTH} ${Math.max(HISTORY_ROW_HEIGHT,events.length*HISTORY_ROW_HEIGHT)}`} preserveAspectRatio="none" aria-label="같은 명세의 변경 관계" role="img">
