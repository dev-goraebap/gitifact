import type { BrowserChangeV2, BrowserSessionV3, SpecEvent, SpecFeature, SpecSnapshot } from '@gitifact/contracts';
...
export function EventDetail({event:e,change,features,session}: {event:SpecEvent;change:UseQueryResult<BrowserChangeV2>;features:SpecFeature[];session:BrowserSessionV3}) {
  useLanguage();
  ...
  return <VStack gap={5} className={styles.readingPane}>
      ...
      <CommitSource session={session} commit={e.commit}/>
      {e.kind==='wiki'
