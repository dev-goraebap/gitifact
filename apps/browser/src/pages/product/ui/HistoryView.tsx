...
export function HistoryView({events,features,search,change}: {events:SpecEvent[];features:SpecFeature[];search:ProductSearch;change:(s:ProductSearch)=>void}) {
 ...
 const filtered=events.filter(e=>(!search.feature||e.before?.specId===search.feature||e.after?.specId===search.feature)&&(!search.author||e.email===search.author)&&(!search.kind||e.types.includes(search.kind as keyof typeof names))&&(!search.q||[e.id,e.before?.title,e.after?.title].join(' ').toLowerCase().includes(search.q.toLowerCase())));
 ...
 const detail=selected?<EventDetail key={selected.key} event={selected} close={()=>change({...search,selected:undefined})}/>:undefined;
 const list=<VStack gap={0} className={styles.timeline}>
  ...
  {!filtered.length&&<PageState kind={events.length?'search':'empty'} title="표시할 요구사항 이력이 없습니다." description={events.length?'필터를 바꾸거나 이전 이력을 더 불러오세요.':'에이전트와 요구사항을 정리하고 커밋하면 이곳에서 변경 이력을 볼 수 있습니다.'}/>}
  {hasConnections&&<Text type="supporting" color="secondary">선은 현재 목록에 있는 같은 R-ID의 변경만 연결합니다.</Text>}
 </VStack>;
