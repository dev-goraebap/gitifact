...
export function ProductPanel({session,view,featureId,email,documentId,search,change}:ProductProps&{session:BrowserSessionV2}) {
 ...
 {view==='features'&&<><Selector label={t('filters.design')} isLabelHidden value={search.design??''} options={[{value:'',label:t('filters.allDesigns')},{value:'yes',label:t('filters.designWritten')},{value:'no',label:t('filters.designMissing')}]} onChange={design=>change({...search,design:design||undefined})}/>
 <Selector label={t('filters.contributor')} isLabelHidden value={search.author??''} options={[{value:'',label:t('filters.allContributors')},...first.contributors.map(p=>({value:p.email,label:p.name}))]} onChange={author=>change({...search,author:author||undefined})}/>
 <Selector label={t('filters.sort')} isLabelHidden value={search.sort??'recent'} options={[{value:'recent',label:t('filters.sortRecent')},{value:'requirements',label:t('filters.sortRequirements')},{value:'title',label:t('filters.sortTitle')}]} onChange={sort=>change({...search,sort:sort==='recent'?undefined:sort})}/></>}
