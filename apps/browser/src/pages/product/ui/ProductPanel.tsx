...
export function ProductPanel({session,view,featureId,email,documentId,search,change}:ProductProps&{session:BrowserSessionV1}) {
 ...
 const title={history:'활동',features:'제품 기능',contributors:'참여자',product:'제품 개요',guides:'지침'}[view];
 const centered=true;
 ...
 // Column browsing keeps the guide beside its folders, so only the list layout counts as a separate page. The product page has no list or filters at all.
 ...
 const filters=ready&&!detailPage&&!productPage&&<HStack gap={3} wrap="wrap" className={`${styles.filters} ${centered?styles.filtersSticky:''}`}><TextInput label="검색" isLabelHidden placeholder={view==='features'?'기능·요구사항 검색':view==='contributors'?'이름 또는 이메일 검색':guides?'문서 제목·경로 검색':'이름 또는 ID 검색'} value={search.q??''} hasClear onChange={q=>change({...search,q:q||undefined},true)}/>
 ...
 {guides&&<SegmentedControl label="보기 방식" size="sm" value={search.view==='list'?'list':'column'} onChange={value=>change({...search,view:value==='list'?'list':undefined,folder:undefined})}><SegmentedControlItem value="column" label="열"/><SegmentedControlItem value="list" label="목록"/></SegmentedControl>}
 </HStack>;
 ...
 {!detailPage&&<VStack gap={1} className={styles.pageTitle}><Heading level={1}>{title}</Heading>{guides&&<Text type="supporting" color="secondary">구현 방식을 정하는 아키텍처·코드 규칙 문서입니다. 에이전트가 설계할 때 참고합니다.</Text>}</VStack>}
