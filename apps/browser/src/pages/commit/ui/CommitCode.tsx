...
/**
 * The source files a commit changed: each with how it changed and how many lines, and the chosen one's diff. The first
 * file opens when none is named. Documents are left out, being the documents tab.
 */
export function CommitCode({ session, commit, file, href }: { session: BrowserSessionV3; commit: string; file: string | undefined; href: (path: string) => string }) {
  ...
  if (!data) return <VStack gap={2} role="status" aria-label={t('source.listLoading')}>
    {[92, 86, 74].map((w, i) => <Skeleton key={i} index={i} width={`${w}%`} height="var(--spacing-4)"/>)}
  </VStack>;
  if (!data.files.length) return <Text color="secondary">{t('source.none')}</Text>;
  const chosen: CommitFile = data.files.find(f => f.path === file) ?? data.files[0]!;
  return <SplitReader label={t('source.title')}
    summary={<Text type="supporting" color="secondary">{t('source.count', { count: data.total })}{data.total > data.files.length ? ' · ' + t('source.shown', { count: data.files.length }) : ''}</Text>}
    items={data.files.map(f => { const { name, folder } = split(f.path);
