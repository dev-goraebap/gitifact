...
import { browserSpecsV2, type BrowserSessionV2 } from '@gitifact/contracts';
...
export const specsOptions = (session: BrowserSessionV2) => infiniteQueryOptions({
  queryKey: ['browser-specs', 2, window.location.origin, session.sessionId, session.repository.worktreeKey],
  ...
  queryFn: async ({ signal, pageParam }) => {
    ...
    const parsed = browserSpecsV2.safeParse(data);
