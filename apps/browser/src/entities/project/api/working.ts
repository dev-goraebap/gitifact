 ...
 * Two sources: the checkout answer's `working` flag from the cache the product pages fill (never fetched here, so repositories without a
...
export function useWorkingChanges(): boolean | undefined {
  ...
  const specs = useQuery({ ...checkoutOptions(session.data ?? noSession), enabled: false });
