...
export function RequestState({ error, retry }: { error?: Error | null; retry?: () => void }) {
  ...
  return <VStack padding={6} gap={6} role="status" aria-label="프로젝트 불러오는 중" aria-busy="true" style={{minHeight:'24rem', opacity:visible?1:0, transition:'opacity .2s ease-out'}}>
