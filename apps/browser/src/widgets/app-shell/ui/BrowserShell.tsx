...
export function BrowserShell() {
  ...
  useSmoothWheel(card);
  // A move to another screen keeps this one in view while the next one's reads are primed by its route; past 100ms the
  // rocket veils it, for 300ms at least. A change of search on the same screen is not a move and keeps its own list.
  const moving = useRouterState({ select: (s) => s.status === 'pending' && !!s.resolvedLocation && s.location.pathname !== s.resolvedLocation.pathname });
  const veiled = useLoadingHold(moving, { delay: 100, minimum: 300 });
  ...
  return (
    ...
    >
      <VStack gap={0} className={styles.frame}>
        <VStack gap={0} ref={card} className={styles.card} data-scroll-restoration-id="content" aria-busy={moving}>
          <Outlet />
        </VStack>
        <PageLoader isVisible={veiled} place="card"/>
        <SearchPalette/>
      </VStack>
    </AppShell>
  );
