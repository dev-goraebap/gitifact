...
test('loading, successful refresh and failure retain the last observed data', async ({ page }) => {
  ...
  await page.route('**/api/v1/status', async route => { await initial; await route.fulfill({ json: current }); });
  ...
  await page.route('**/api/v1/status/refresh', async route => {
    posts++;
    if (posts === 1) {
      started(); await refreshGate;
      current = repositoryStatusSuccessV1.parse({ ...status,
    ...
    } else await route.fulfill({ status: 503, json: repositoryStatusFailureV1.parse({
...
test('wrong worktree and incompatible contracts cannot be shown as current data', async ({ page }) => {
  ...
  await page.route('**/api/v1/status', route => route.fulfill({
    json: wrong ? { ...status, repository: { ...status.repository, worktreeKey: 'worktree:' + 'd'.repeat(64) } } : status,
  }));
  await page.goto('/');
...
test('connection failure and empty result are distinct', async ({ page }) => {
  ...
  await expect(page.getByRole('table')).toHaveCount(0);
  await mockApi(page);
  await page.route('**/api/v1/status', route => route.fulfill({ json: repositoryStatusSuccessV1.parse({
    ...status, changes: [], summary: { staged: 0, unstaged: 0, untracked: 0, conflicted: 0 },
  }) }));
  await page.getByRole('button', { name: '다시 연결' }).click();
