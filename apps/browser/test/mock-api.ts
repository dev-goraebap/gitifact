...
export type Fixture = Omit<BrowserSpecsV5, 'version' | 'features' | 'documents' | 'problems'> & { version?: number; features: LooseFeature[];
  documents: (Omit<BrowserSpecsV5['documents'][number], 'description'> & { description?: string })[]; problems?: BrowserSpecsV5['problems']; events: SpecEvent[] };
...
/** The checkout part of a fixture, as /api/v1/specs answers it: the shorthand filled out to the v5 shape. */
export const checkoutOf = ({ events: _events, features, documents, problems, ...rest }: Fixture): BrowserSpecsV5 => ({ ...rest, version: 5,
  ...
  documents: documents.map(d => ({ description: d.title, ...d })), problems: problems ?? [] });
...
export async function serve(page: Page, data: Fixture) {
  ...
  await page.route('**/api/v1/history/summary*', route => {
    ...
    return route.fulfill({ json: { contract: 'browser-history-summary', version: 2, sessionId: session.sessionId, head: data.head, total: events.length,
  ...
  await page.route('**/api/v1/search*', route => {
    ...
    const records = [
      ...
      ...checkout.documents.map(d => ({ id: d.id, kind: 'document', title: d.title, where: d.path.replace(/^\.gitifact\/wiki\//, ''), body: d.body, documentId: d.id })),
...
export const readmeBody = '요구사항과 변경 이유를 **Git**에 연결합니다.\n\n![로고](../assets/logo.png)\n\n[레이아웃 지침](frontend/layout.md) · [개발 환경](../../docs/development.md) · [없는 페이지](missing.md) · [검색어 입력](../spec/search/requirements/r-abcdefghij.md)';
export const specs: Fixture = {
 contract:'browser-specs',version:5,sessionId:session.sessionId,head:'c'.repeat(40),observedAt:'2026-09-14T00:00:00Z',working:false,
