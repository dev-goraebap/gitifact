...
test('built app loads Astryx and supports navigation, reload, and history', async ({ page }) => {
  ...
  await page.goto('/');
  await expect(page.getByRole('heading', { level: 1 })).toHaveText('프로젝트의 기록을 이어갑니다');
  const button = page.getByRole('button', { name: 'tryce 소개' });
  await expect(button).toBeVisible();
  // Confirms that the shipped component CSS and theme have both loaded.
  expect(await button.evaluate((element) => getComputedStyle(element).getPropertyValue('--color-text-primary').trim())).not.toBe('');
  await button.focus();
