...
요구사항 R-ID와 설계 S-ID를 각각 비교해 생성·수정·삭제·이동을 표시한다. 같은 커밋 항목은 연속 표시하고 각 행에서 전후 원문·이유를 연다. 그래프는 같은 항목의 변경만 연결하며 Git 브랜치 병합을 요구사항 관계로 꾸미지 않는다. 현재 별도 분리·통합 관계 데이터는 제공하지 않는다.

종류·기능·작성자·검색 필터는 불러온 범위에 적용한다. 더보기는 기존 목록에 다음 페이지를 추가하며 실패 시 재시도할 수 있다. 모든 이력을 처음부터 한 번에 분석하지 않는다.
