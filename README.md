...
현재 배포판은 Node.js 24.x와 Git이 필요하며 Windows에서 실행을 검증했습니다. 프롬프트의 `prototype`은 0.1.0에서 note를 사용하기 위한 기존 설정 이름입니다. 프로젝트의 개발 단계를 뜻하도록 강요하려는 것은 아니며, 앞으로 기록·확인 정책을 중심으로 바꿀 예정입니다.
...
새 정책은 **자동모드(`auto`) / 승인모드(`approval`)**로 설계하고 있습니다. 신규 도입의 기본인 자동모드는 의도가 명확하면 별도의 기록 승인을 기다리지 않고 작업을 이어갑니다. 승인모드는 관련 요구사항 초안을 묶어 보여주고 확인받은 뒤 진행합니다. 두 모드 모두 제품 동작이 불명확하면 질문합니다. 현재 0.1.0에는 아직 적용되지 않았습니다.
...
커밋은 프로젝트의 기존 정책을 따르고, 별도 정책이 없으면 관련 작업 단위에서 자동 커밋하도록 설계합니다. 관련 기록과 코드는 같은 변경으로 담을 수 있지만, 관련 없는 변경은 섞지 않습니다. 사용자가 stage한 내용은 임의로 바꾸지 않으며, 기록 파일을 저장한 것과 Git에 커밋한 것도 구분합니다. 자동 푸시는 포함하지 않습니다.
