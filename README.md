...
> 0.2.1에서는 요구사항을 `.tryce/spec/`에 기록합니다. 자동모드·승인모드, 요구사항 초안과 묶음 확인, Git 커밋 계획·실행을 지원합니다. 요구사항 GUI와 자동 구현·검증 완료 판정은 아직 제공하지 않습니다.
...
배포된 0.2.1에는 요구사항 GUI가 포함되지 않습니다. 이 저장소의 개발 버전에는 읽기 전용 브리핑·요구사항 목록과 상세·판단 기록 화면을 추가했습니다. 자동 구현·검증 완료 판정, 대체·폐기 상태 전이, 전체 Git 이력 검사와 훅은 아직 제공하지 않습니다. 저장 상한과 실패 복구, 정확한 지원 범위는 [실행 형식](docs/specs/workflow-format.md)에 정리했습니다.
