...
사용자가 선택한 정책에 따라 에이전트가 별도의 확인을 기다리지 않고 기록하며 작업을 이어가거나, 요구사항을 확정하기 전에 관련 초안을 묶어 보여주고 승인을 받는 흐름을 지향합니다. 이 선택은 프로젝트가 프로토타입인지 운영 중인지와 별개입니다. 어떤 방식이든 중요한 발견·제약·기각 이유를 남기고, 실제 사용자 승인을 받은 내용은 따로 구분합니다. 세부 정책과 단계 이름은 아직 정하지 않았습니다.

승인이 필요한 경우에는 요구사항의 내용뿐 아니라 사용자가 무엇을 승인했는지도 함께 남깁니다. 기록됨, 승인됨, 구현됨, 검증됨을 구분해 사람과 에이전트가 진행 상태를 명확히 추적할 수 있어야 합니다.

커밋은 프로젝트의 기존 정책을 따릅니다. 관련 기록과 코드는 같은 변경으로 담을 수 있지만, 관련 없는 변경은 섞지 않습니다. 기록 파일을 저장한 것과 Git에 커밋한 것도 구분합니다.
