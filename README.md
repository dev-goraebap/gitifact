...
> npm 배포판은 0.1.0입니다. 개발 버전 0.2.0-dev.0에는 자동모드·승인모드, 요구사항 초안과 묶음 확인, Git 커밋 계획·실행을 추가했습니다. 아래 시작 프롬프트는 현재 배포판에 맞춰 두었으며 새 기능은 아직 npm에 게시하지 않았습니다.
...
새로 도입하는 프로젝트는 현재 0.1.0의 prototype 기록 모드로 초기화하고
note 기록을 활성화해주세요. 이미 도입돼 있다면 설정·모드·기준선을 유지하고,
호환되지 않는 경우 임의로 전환하지 말고 필요한 조치를 알려주세요.
...
생략된 내용 중 필요한 원문도 읽어주세요. 이후 개발 중 중요한 발견·제약·기각
이유는 제가 매번 지시하지 않아도 note로 남겨주세요. 실제 적용한 범위와
남은 문제가 있으면 짧게 알려주세요.
```

현재 배포판은 Node.js 24.x와 Git이 필요하며 Windows에서 실행을 검증했습니다. 프롬프트의 `prototype`은 0.1.0에서 note를 사용하기 위한 기존 설정 이름입니다. 개발 버전에서는 프로젝트 단계와 별개인 auto/approval을 사용합니다. 기존 프로젝트의 모드는 업데이트만으로 바뀌지 않습니다.
...
개발 버전은 **자동모드(`auto`) / 승인모드(`approval`)**를 지원합니다. 신규 도입의 기본인 자동모드는 의도가 명확하면 별도의 기록 승인을 기다리지 않고 작업을 이어갑니다. 승인모드는 관련 요구사항 초안을 묶어 보여주고 확인받은 뒤 진행합니다. 두 모드 모두 제품 동작이 불명확하면 질문합니다. 현재 npm의 0.1.0에는 적용되지 않았습니다.
...
배포판 0.1.0은 설정·기준선, 발견·제약·기각 이유, 세션 브리핑, Git 상태 조회, 읽기 전용 브라우저와 프로젝트 스킬 설치를 지원합니다.

개발 버전에서는 에이전트가 요구사항 초안을 모으고 묶음 단위로 확정·확인한 뒤 Git에 기록하는 흐름을 GUI 없이 사용할 수 있습니다. 모드를 바꿔도 과거 승인을 만들지 않으며 확인 뒤 수정한 내용은 다시 초안으로 구분합니다. CLI는 원문·참조·해시와 커밋 범위를 검사하고, 제품 의미와 실제 확인 여부는 에이전트가 판단합니다.

요구사항 GUI, 자동 구현·검증 완료 판정, 대체·폐기 상태 전이, 전체 Git 이력 검사와 훅은 아직 제공하지 않습니다. 저장 상한과 실패 복구, 정확한 지원 범위는 [실행 형식](docs/specs/workflow-format.md)에 정리했습니다.
