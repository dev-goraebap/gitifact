...
브라우저 화면만 바뀐 패치 배포다. 활동 화면이 커밋 하나를 한 항목으로 두고 변경 이유를 그 이유로 바뀐 기록들 위에 한 번만 적는다. 기능별 요구사항 목록은 기능 아래에 그 기능의 요구사항을 한 줄씩 잇고 누르면 프래그먼트로 상세의 그 절을 연다. 요구사항에서 설계 절로 가는 링크, 절 제목의 형광펜 표시, 열 머리 정렬, 왼쪽 메뉴의 정렬이 함께 들어간다. CLI 명령과 저장 규약은 바뀌지 않았다. 설계와 측정은 [개발 환경](development.md)의 2026-09-20 항목을 따른다.
