...
기본 경로 /는 요구사항 이력, /features는 제품 기능, /contributors는 참여자다. 화면 이름은 2026-09-15 사용자 요청으로 기여자에서 참여자로 바꿨고 경로는 유지한다. 같은 날 상세 페이지를 경로 계층으로 옮겼다. 기능 상세는 /features/$featureId, 참여자 상세는 /contributors/$email이며 이메일은 percent 인코딩한다. 이전의 ?feature=·?author= 진입은 리다이렉트하지 않고 목록으로 취급한다. 없는 ID는 찾을 수 없음 상태로 안내한다. 구형 브리핑·승인·note 페이지를 제거했고 /requirements는 제품 기능의 기존 URL 진입으로 유지한다. pages/product 슬라이스 안에서 화면을 조합하고 공통 세션·명세 Query는 entities/project가 소유한다.
...
현재 기능 목록은 작업 폴더 기준이며 미커밋 명세를 표시한다. 이력은 Git 커밋 기준이다. 참여자는 현재 HEAD에서 도달 가능한 최근 최대 10,000개 커밋의 mailmap 적용 author 이름·이메일로 집계한다. 초과 시 제한을 표시한다. 기능 참여·최근 명세 활동은 불러온 이력 범위이며 커밋 수를 요구사항 기여 수나 승인으로 해석하지 않는다. 이메일 기반 대칭 아바타는 로컬에서 생성하며 외부 이미지 조회를 하지 않는다.
