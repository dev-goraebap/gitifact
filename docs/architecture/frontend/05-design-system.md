...
Gentask의 05-design-system.md를 Gitifact에 적용한다. Astryx의 설치 버전이 API 기준이며 제품별 배치 규칙은 이 문서를 따른다.
...
요구사항 본문은 Markdown으로 읽어 표시하되 현재 원본은 .gitifact/spec/<기능>/requirements.md이다. Astryx Markdown의 안전한 렌더링을 사용하고 원시 HTML 실행을 허용하지 않는다. 이 화면에서는 편집기·업로더를 설치하지 않는다.
