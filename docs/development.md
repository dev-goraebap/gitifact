...
검증: `pnpm check` 통과(브라우저 65, CLI 128). 브라우저 테스트에 한 이유가 기록 셋 위에 한 번만 적히는지, 커밋 하나가 항목 하나인지, 날짜가 바뀔 때만 날짜가 붙는지 검사하는 건을 더했고, 목록 줄 수를 세던 기존 세 건은 기록 목록을 세도록 고쳤다. 실제 CLI 서버에서 라이트·다크·760px과 필터·선택·더보기·골격을 확인했고 가로 스크롤과 콘솔 오류는 없다. 커밋·푸시는 하지 않았다. 캡처는 `.tmp/activity/`에 있다.
