...
브라우저는 결정기록 목록(`/records`), 기록 상세(`/records/<ID>`), 커밋 상세(`/records/commits/<커밋>`)로 나눴고 서버에 `/api/v1/record`를 더했다. 제품 개요는 대시보드(`/dashboard`)로 바꿨다. 사용자 피드백으로 지침 상세의 흔들림(파일 미리 읽기, 이전 내용 유지, 다이어그램 보관), 지침 상세의 뒤로가기, 필터 아이콘, 좁은 화면의 여백·필터·기능 표를 고쳤다. 촬영 중 0.7 이유가 문서마다 반복되던 결함을 찾아 고쳤고 소개 스크린샷 세 장을 다시 찍었다(`docs/readme-screenshots.md`). 에이전트가 마친 작업을 커밋하지 않은 채 다음 작업으로 넘어가면 커밋을 한 번 제안하도록 블록과 커밋 가이드를 고쳤다.

검증: `pnpm check` 통과(core 41, CLI 143, 브라우저 90, contracts 9, intro 2, 패키지 설치 시험). 이후 브라우저·소개 커밋마다 브라우저 테스트 90개와 `docs check`(문서 88개) 통과. 커밋은 결정 단위로 나눴고, 128개 한도 때문에 첫 커밋 `424c7cf`만으로는 브라우저가 빌드되지 않는다.
...
기계적인 분할은 일회성 스크립트(`.tmp/0.8.0-design/tools/migrate-this-repo.mjs`, slug·설명은 손으로 쓴 `migrate-map.json`)로 했다. CLI에는 변환 코드를 두지 않는다는 원칙에 따른 것이다. 검증: `docs check` 문제 없음(문서 82개), 0.7 파서로 읽은 전환 전 커밋과의 대조(`verify-migration.mjs`)에서 개수·ID·제목·순서·본문·이유 문장이 모두 일치했다. `docs history`로 전환 전 활동이 같은 R-ID에 이어지는 것과, 브라우저 API에서 기능 9개·위키 19쪽·활동 227건이 문제 없이 보이고 마이그레이션 커밋은 활동에 나오지 않는 것을 확인했다. 캐시가 빈 상태의 첫 이력 조회는 약 8.6초 걸렸다. 옛 브라우저 색인 `.git/gitifact/`는 지우지 않았다.

프로젝트 사용 빌드는 `pnpm cli`(`apps/cli/dist/main.js`, `35c6dfe`에서 빌드, SHA-256 `5e6919e8b84201813945ea07764d6993c53e40356d00cf2041abd27c297cc6ba`)로 지정한다. 이 빌드는 6단계의 `docs`·`changes`·`guide` 명령을 담고 `pnpm check`를 통과했다. 배포된 0.7.1은 schemaVersion 3을 읽지 않으므로 이후 조회·커밋에 쓰지 않는다. AGENTS.md의 GITIFACT 블록은 이 빌드의 `update`로 갱신했다.

## 2026-09-21 0.7.1 사용 빌드

버전과 한국어·영어 패치노트를 0.7.1로 맞췄다. 타입 검사·빌드, core 29건·contracts 9건·intro 2건을 통과했다. 브라우저 85건 중 한 건은 종료 후 임시 폴더 삭제가 EBUSY로 실패해 단독 재실행으로 통과했다. CLI 146건 중 한 건은 다국어 테스트가 0.7.0을 고정해 기대하던 문제였으며, package.json의 버전을 읽도록 고쳐 해당 파일 6건을 재검사해 통과했다. 패키지 오프라인 설치 검사도 통과했다. 전체 pnpm check의 최초 실행과 후속 재검사를 구분하며 제품 런타임은 이 검증 중 수정하지 않았다.

검증한 apps/cli/dist/main.js(SHA-256 5e3cb9cbf8fa301732c809363eb633d7141e26b4359ebbf5186ef2e757a164f2)를 프로젝트 조회·저장·커밋 사용 대상으로 지정한다. AGENTS.md 블록도 이 빌드의 update로 갱신한다. npm 게시 결과와 Git 태그는 releases.md에 기록한다.

## 2026-09-21 병합 이력의 원본 작성자

활동·개요·위키 이력이 first-parent 계보만 읽어 원본 작업자를 병합 담당자로 표시하던 문제를 고쳤다. 모든 부모의 원본 커밋을 읽고 일반 병합의 중복 기록을 제외한다. 두 부모 병합에서 추가로 수정한 기록은 재병합 비교로 골라 병합 커밋에 남긴다. 가져온 이유는 원본 커밋에, 병합에서 처음 쓴 이유는 병합 커밋에 연결한다. 색인 형식을 2로 올려 이전 작성자·집계·검색 행은 다음 서버 조회 때 다시 만든다.

임시 저장소의 회귀 검사 9건을 통과했다. SHA-1·SHA-256, 원본 작성자·이유·검색·필터·개요·상세, 브랜치에서 main을 가져온 뒤 재병합, 한 요구사항 안의 자동 합성, 충돌 해결, 병합 중 수정·삭제, 위키 이동·설계 삭제, 자체 되돌리기, fast-forward·스쿼시·세 부모 병합, CRLF·따옴표 ID, 구형 색인 재생성과 원본 파일·Git 데이터 보존을 확인했다. 기존 브라우저 이력 검사 9건도 통과했다.

safety-solution-webapp은 .tmp/merge-attribution-safety 복사본에서 검사했다. 복사 시점 HEAD 9348f6b의 이력 381건을 새 색인으로 읽는 데 약 3.7초가 걸렸다. 온보딩 위키(e7658f104)와 TBM 요구사항(e26f7ae8), 최근 위키 변경(46b37c731)이 강수정의 원본 커밋으로 표시되며 80d09b51 병합의 중복 위키 기록은 없다. 원본 프로젝트의 진행 중인 변경은 건드리지 않았다.

구현 중 pnpm check를 통과했고 CRLF·따옴표 ID 보완 후 병합 회귀 9건을 다시 통과했다. 커밋 전 최종 pnpm check에서는 타입 검사·빌드, core 29건·contracts 9건·intro 2건·브라우저 85건과 CLI 146건 중 145건이 통과했다. SHA-256 설계 수명 주기 테스트의 Git reset이 10초 제한을 넘겨 실패해, 해당 파일의 3건을 단독 재실행하여 모두 통과했다. 이어 최종 패키지 오프라인 설치 검사도 통과했다. 검증한 apps/cli/dist/main.js의 SHA-256은 7edc2b23cdf43f83c7e1af2e1c2bea54d98055c3794abb07c4a37b14116dd06d이며 프로젝트 조회·저장·커밋 사용 대상으로 지정한다. 이번 명세 저장은 빌드 전 지정본 0cf6849b로 수행했다.

AGENTS.md의 GITIFACT 블록은 검증된 로컬 빌드의 pnpm cli update로 0.6.2에서 0.7.0 지침으로 갱신했다. npx 실행·세션 시작 버전 확인 안내를 반영하고, 블록 밖의 프로젝트 전용 pnpm cli 실행 규칙과 나머지 내용이 그대로임을 확인했다.

Git 2.53.0에서 검사했으며 두 부모 병합 판별에는 remerge-diff 지원이 필요하다. 세 부모 이상 병합은 모든 부모와 다른 기록을 남기지만 같은 기록 안의 자동 합성과 수동 수정을 구분하지 못한다. 스쿼시로 사라진 원본 작성자는 복원하지 않는다. 푸시·배포는 하지 않았다.

## 2026-09-21 기능별 요구사항 헤더 고정

페이지 이동 버튼이 있는 긴 기능 목록에서 프로젝트명·검색·새로고침 헤더가 스크롤 중 위로 사라졌다. 푸터를 바닥에 두는 flex 확장 규칙이 안쪽 컨테이너의 축소도 허용해 본문은 넘치지만 sticky 헤더의 부모 높이는 화면 크기로 줄어들었다. 페이지와 column·content·featureTable이 축소되지 않게 해 목록 길이만큼 부모도 늘어나도록 고쳤다. 짧은 마지막 페이지의 푸터 배치는 유지한다. 관련 브라우저 설계에 원인을 남겼다.

검증: 수정 전 데스크톱·좁은 화면의 라이트·다크 네 조건에서 헤더 위치 검사 실패를 확인했다. 수정 후 기능 목록 14건과 최종 브라우저 전체 85건이 통과했다. 목록 중간·끝·처음에서 헤더 위치와 다른 내용에 가려지지 않는지 확인하고, 긴 페이지·짧은 페이지의 푸터 검사도 통과했다. 커밋 전 pnpm check도 통과했다. 타입 검사·빌드, core 29건·contracts 9건·intro 2건·브라우저 85건·CLI 137건과 패키지 오프라인 설치 검사를 수행했다. CLI 실행 번들 해시는 앞선 지정과 같으며 수동 화면 검증은 사용자에게 맡겼다. 푸시는 하지 않았다.

## 2026-09-21 세션 시작 버전 확인과 브라우저 알림 제거

브라우저의 업데이트 버튼·대화상자·서버 레지스트리 조회·세션 반복 조회를 제거했다. 현재 버전과 패치노트 링크는 유지한다. browser-session은 update 필드가 없는 v3으로 바꾸고 이전 v2 스키마는 보존했다. 불필요해진 browser --no-update-check 옵션도 제거했다.

update --check는 프로젝트를 읽거나 쓰지 않고 실행 버전과 npm 최신 정식 버전을 비교한다. 별도 update-check v1 계약으로 결과와 새 버전 실행 명령을 반환하며 --commit과 함께 쓸 수 없다. 한국어·영어 블록과 workflow는 새 세션에서 지정 버전으로 한 번 확인하고 사용자 동의 후 갱신하도록 안내한다. 거절·실패·비활성화 시 지정 버전으로 계속하며 같은 세션에서 다시 묻지 않는다. 업데이트 동의는 커밋·푸시 권한을 포함하지 않는다.

README와 시작하기의 일상 예시는 npx gitifact로 줄였으며 도입·업데이트만 @latest를 쓴다. --yes와 정확한 버전은 에이전트 블록에 둔다. init·update의 npx 안내와 project-init v6·update v4 변경도 함께 포함한다.

검증: pnpm check 통과. 타입 검사·빌드, core 29건·contracts 9건·intro 2건·브라우저 81건·CLI 137건과 패키지 오프라인 설치 검사를 수행했다. --check의 새 버전·최신·조회 실패·시간 초과·비활성화, 저장소 밖 실행, 오래되거나 깨진 블록·파일·index·HEAD 보존, --commit 충돌과 기존 갱신·커밋 동작을 확인했다. 브라우저는 외부 요청·알림·반복 조회 제거와 패치노트 유지를 검사했다. 수동 화면 검증은 사용자에게 맡겼다. 실제 에이전트의 질문·동의 행동은 자연어 지침이며 CLI 테스트가 이를 보장하지 않는다.

검증한 CLI는 apps/cli/dist/main.js, SHA-256 0cf6849b8f4602c7fc53f42d6896dffc3553f5e99867af1fba242ef5ccd6c70e다. 이 빌드를 프로젝트 조회·저장·커밋 사용 대상으로 지정한다. 명세 저장에는 앞서 지정한 32c2243ad57d40fcbccd800e7202dc7036b5885d52c207f537ed92818d171882 빌드의 보존본(.tmp/session-check-record-cli/main.js)을 사용했다. npm 버전은 올리지 않았고 푸시·배포는 수행하지 않았다.

## 2026-09-21 npx 실행·업데이트 안내 통일

시작하기와 npm README의 사용자용 명령을 npx로 맞췄다. 처음 도입·업데이트는 @latest, 평소 작업은 프로젝트 GITIFACT 블록의 지정 버전을 사용한다. init·update의 텍스트 출력과 브라우저 업데이트 창도 새 버전의 npx 명령을 안내한다. 프로젝트 의존성과 전역 설치는 선택 사항으로 유지한다.

JSON 응답에 install.npx를 추가해 project-init은 v6, update는 v4로 올렸다. 기존 v5·v3 스키마는 유지하며 install.npmGlobal도 선택지로 제공한다. 저장 규약과 사용자 문서는 바꾸지 않았다. 관련 도입·시작하기·업데이트 명세와 설계를 함께 갱신했다.

타입 검사·빌드, core 29건·계약 8건·소개 문서 2건이 통과했다. 화면 검사는 변경 전 문구를 기대하던 테스트와 번역 치환을 수정한 뒤 Chromium 페이지 종료로 한 번 실패했고, 동시 실행 수를 4로 낮춰 81건 모두 통과했다. CLI 전체 135건 통과 후 지침 파일 누락 안내의 npx 명령을 보완해 CLI를 다시 빌드하고 업데이트·다국어 12건을 재검사했다. 최종 패키지의 오프라인 설치·npx 실행·초기화·지침 갱신·명세 저장·커밋·조회·브라우저 검사를 통과했다. 전체 pnpm check가 한 번에 통과한 것은 아니며 실패·중단된 검사는 위와 같이 별도로 완료했다.

최종 CLI 번들 SHA-256은 `32c2243ad57d40fcbccd800e7202dc7036b5885d52c207f537ed92818d171882`이며 `apps/cli/dist/main.js`를 프로젝트 조회·저장·커밋 사용 빌드로 지정한다. 명세 저장에는 빌드 전 0.7.0 지정 CLI를 사용했다. 사용자 프로젝트와 전역 설치는 변경하지 않았고 이번 변경은 아직 커밋·배포하지 않았다.
