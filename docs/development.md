...
현재 CLI의 `init`으로 설정·도입 기준선을 만들고 `note`로 프로토타입 기록을 추가·조회한다. `brief`는 작업 시작에 필요한 현재 자료와 원문 위치를 제공한다. `status`와 `browser`는 Git 상태를 조회한다. `skills install/sync/remove`는 스킬 원본 설치와 로컬 복사본 관리를 지원한다. 요구사항 기록·검사와 훅은 지원 범위 밖이다.
