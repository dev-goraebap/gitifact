...
공용 스킬은 기존 AGENTS.md·CLAUDE.md의 적용 가능한 지침을 읽고 따른다. 설치 명령은 이 파일들을 생성하거나 스킬 연결 문구를 삽입하지 않는다. README의 도입 프롬프트를 받은 에이전트가 현재 환경의 지침 파일과 참조 관계를 확인해 기존 정책을 보존하고 짧은 스킬 연결 안내를 추가한다. 이미 직접 또는 다른 지침을 통해 연결돼 있으면 중복 추가하지 않는다. 파일이 없으면 현재 에이전트가 사용하는 파일 하나를 만들고 설치 결과의 실제 경로를 연결한다. 정책 본문을 복제하거나 두 파일을 일괄 생성하지 않는다.

현재 세션에서는 설치 직후 스킬을 직접 읽는다. 이후 세션은 프로젝트 지침의 연결과 에이전트의 스킬 인식을 사용한다. 이 절차는 에이전트의 문서 편집이며 CLI가 지침 파일의 소유권·자동 동기화·삭제를 관리하는 기능은 아니다. 이 저장소의 개발 예외는 AGENTS.md에 두며 공용 스킬에 배포하지 않는다.
