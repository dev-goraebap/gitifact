...
저장 경로와 마커의 변환은 2026-09-15 `gitifact migrate`로 구현했다. `.tryce`를 `.gitifact`로 이름만 바꾸고 requirements.md·design.md의 줄 첫머리 `tryce-*` 마커를 `gitifact-*`로 바꾼다. 코드 블록 안의 예시와 history.jsonl은 그대로 두며 구형 tryce.json은 옮기기만 하고 읽지 않는다. 전환 결과는 `spec commit`에 이전 경로의 삭제와 새 경로의 추가를 함께 선택해 커밋한다. CLI는 과거 커밋의 `.tryce` 경로와 `tryce-*` 마커를 읽기만 지원하고, 한 커밋에 두 저장 경로가 함께 있으면 거부한다. 저장 경로 변경은 명세 변경으로 보지 않으므로 전환 커밋에는 이유도 트레일러도 생기지 않는다. 이전 Tryce 실행의 잠금 파일은 새 이름과 별개로 계속 검사한다. 기존 S-ID·R-ID·H-ID와 이유 원문·도입 기준선은 보존한다. 단순 문자열 치환으로 사용자 본문이나 이미 커밋된 기록을 바꾸지 않는다. 구형 형식 판별 문자열과 과거 읽기용 이름은 현재 브랜드 표시와 구분한다. 전환 후 과거 명세가 전부 새로 생성된 것으로 보이거나 이력을 읽지 못하는지 시험한다.
...
2026-09-15 패키지·workspace·스킬·저장 경로·마커·트레일러·환경 변수·헤더·화면·테스트·문서를 전환했다. 검증·배포 결과는 [개발 환경](development.md)과 [배포 기록](releases.md)에 남긴다. README와 CLI 안내의 준비 중 표시는 실제 게시 결과에 맞춰 갱신한다.
