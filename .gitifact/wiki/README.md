...
이 위키에는 아키텍처 결정 기록(ADR)과 개발 규칙을 쌓는다.
...
규칙이 바뀌면 해당 페이지를 고친다. 그 변경이 여러 기능에 걸치는 구조·기술 선택이면 결정 기록도 함께 남긴다. 한 규칙 페이지의 범위에 머무는 선택과 기각한 안(라이브러리·글꼴 고르기 등)은 결정 기록 대신 그 페이지 끝의 `## 결정` 표(`| 결정 | 이유 | 기각한 안 |`)에 한 줄씩 둔다. 결정 기록과 달리 이 표는 결정이 바뀌면 그 줄을 고친다. 표에 없는 주제가 생기면 페이지를 만들고 이 표에 올린다. 제품이 무엇이고 누구를 위한 것인지는 위키가 아니라 저장소 루트의 README.md에 있고 원본은 `packages/intro`다.
