...
스크롤은 콘텐츠 카드가 맡는다. 마우스 휠은 Lenis(`shared/lib/smooth-scroll`)로 감속하며 움직인다. 운영체제에서 움직임 줄이기를 켜면 쓰지 않고, 카드 안에서 따로 스크롤되는 영역(위키 트리, 목차, 넓은 표)은 네이티브 스크롤을 둔다(`allowNestedScroll`).
