...
브라우저는 CLI 로컬 서버가 제공하는 읽기 전용 React SPA다. 제품 개요, 요구사항, 프로젝트 위키, 활동, 참여자, Git 상태를 보여 주며 요구사항을 편집하거나 승인을 만들지 않는다. 화면마다의 구성과 동작은 [명세 브라우저 설계](../../spec/browser/design/overview.md)와 그 축 파일들이 설명하고, 이 페이지는 브라우저 코드를 고칠 때 따르는 규칙을 둔다. 설계의 기각한 안과 이유는 그 개요의 결정 표에 있다.

## 경로와 URL 상태

메뉴와 경로의 전체 목록과 순서는 [화면 구성](../../spec/browser/design/ui.md)의 메뉴와 경로 절에 있다. `/`는 `/product`로 replace 리다이렉트한다. 화면의 상태는 종류마다 둘 곳이 정해져 있다.

| 상태 | 둘 곳 |
| :--- | :--- |
| 목록을 대체하는 상세의 대상(기능 ID·참여자 이메일·문서 ID·커밋) | Router 경로 매개변수 |
| 같은 화면 안의 상태(기능 상세의 `tab`, 위키의 `folder`, 검색어 `q`, 필터·정렬·페이지) | Router의 검증된 search params |
...
모든 화면 머리의 문서 검색 입구와 `mod+K`는 같은 검색창을 열고, 검색창에는 별도 주소를 두지 않는다([문서 검색](../../spec/browser/design/search.md)). `/wiki`와 `/wiki/$documentId`는 부모 라우트 `wiki.tsx` 하나가 그리므로 페이지를 옮겨도 트리의 펼침 상태가 유지되고 화면 전체가 다시 나타나지 않는다.
...
| `pages/*` | 화면마다 하나: overview·features·wiki·activity·commit·contributors·git-status·settings·about·getting-started·changelog·not-found |
...
화면 문구는 `shared/i18n`의 `t()`·`tNodes()`로 가져오고, 규칙은 [CLI와 공통 코드](cli.md)의 사용자에게 보이는 문구 절을 따른다. 소개 본문은 `packages/intro/<lang>/intro.md`를 `shared/i18n`이 가져오고, 맨 앞 로고 블록은 같은 패키지의 로고 파일을 번들한 이미지로 보여 준다.
...
같은 HEAD의 이력은 바뀌지 않으므로 새로고침은 체크아웃만 다시 읽는다. 미커밋 명세 변경 표시는 [화면 구성](../../spec/browser/design/ui.md)의 작업 중인 변경 표시 절을 따르며, 셸은 명세 API를 따로 호출하지 않는다.

첫 조회의 골격 표시 시점과 유지 시간은 [조회 상태와 검증](../../spec/browser/design/errors.md)을 따른다(`useLoadingHold`). 골격은 화면별 실제 배치를 그 화면의 여백·간격 클래스를 그대로 써서 본뜬다. 검색어 입력은 타자가 멈춘 뒤 500ms에 반영하며, `shared/lib/search`의 값(`typingDelay`) 하나를 화면 검색창과 검색창 팔레트가 함께 쓴다. 재조회 중에는 현재 목록을 유지한다.
...
제품 개요·활동·커밋 페이지·기능별 요구사항·설정·시작하기의 구성은 [화면 구성](../../spec/browser/design/ui.md)에 있다. 루트 글자 크기와 토큰 확대도 같은 설계의 화면 확대 절을 따르며, 페이지별 zoom이나 root font-size 변경은 두지 않는다.

### 위키 탐색기

위키는 저장소 호스팅의 파일 탐색기 구성을 따른다. 왼쪽 17rem에 위키 전체를 접고 펴는 Astryx TreeList, 오른쪽에 현재 위치 Breadcrumbs와 본문이 있다. 트리와 본문은 따로 스크롤하고, 위치가 바뀌면 본문은 맨 위에서 시작한다.

| 고른 것 | 본문 |
| :--- | :--- |
| 폴더 | 그 폴더의 내용(폴더 먼저, 파일은 이름·제목·최근 변경). 폴더에 README.md가 있으면 목록 아래에 이어 보여 줌 |
| 파일 | 제목·경로·ID·최근 변경·활동 링크와 읽기 본문 |
...
기록 줄은 변경 종류 토큰·명세 종류·`기능 / 문서 제목` 순서이고, 앞의 두 칸이 폭을 지켜 줄끼리 맞춰진다. 기능 제목은 누를 수 없는 옅은 글자이고 줄이 좁으면 먼저 줄어든다. 링크는 문서 제목 하나이며 커밋 페이지의 그 문서 절을 연다. 위키 페이지와 기능 소개는 제목만 둔다.
...
집계 차트에는 차트 라이브러리를 추가하지 않는다. 문서 다이어그램의 mermaid만 [결정 0008](../adr/0008-bundled-diagram-rendering.md)의 예외다. 제품 개요의 부분-전체 막대와 일자별 변경 막대는 인라인 SVG로 그리고, 크기 비교는 Astryx ProgressBar를 쓴다.
...
경로·계약 버전·요청 검사 순서는 [서버 구조와 API](../../spec/browser/design/interface.md)에 있다. 서버 경로는 `server/routes/`의 경로 표에 한 줄씩 둔다(메서드·경로·세션·쿼리 스키마·처리 함수). 쿼리 스키마는 `@gitifact/contracts`에 두어 브라우저와 함께 쓴다. 모든 요청의 공통 검사는 `server/http/guard.ts` 한 곳이고 서버 프레임워크는 쓰지 않는다.
...
이력은 CLI와 함께 쓰는 캐시 `.gitifact/cache/index.db`가 답하고, git은 캐시에 없는 커밋을 읽을 때만 부른다([결정 0011](../adr/0011-cache-in-gitifact-folder.md)). Windows에서 git 프로세스는 하는 일과 무관하게 한 번에 약 80ms이므로, 기록 수나 파일 수만큼 git을 띄우는 읽기를 새로 만들지 않는다.

## 문서 렌더링

Markdown은 Astryx의 안전한 렌더링을 쓰고 원시 HTML을 실행하지 않는다. 코드·다이어그램·알림의 표시와 mermaid 설정은 [문서 표시](../../spec/browser/design/document.md)를, 쓸 수 있는 표기는 [문서 표기](markdown-notation.md)를 따른다. 테마 토큰은 `light-dark(a, b)` 문자열일 수 있으므로, CSS 변수를 스스로 읽지 못하는 라이브러리에는 토큰의 선언값이 아니라 그 토큰으로 칠한 요소의 계산된 색을 넘긴다. 테스트도 계산된 색을 비교한다.
...
언어 결정 순서와 구독 방식은 [표시 언어 설계](../../spec/localization/design/overview.md)를 따른다. 설정에서 브라우저 설정·한국어·영어를 고르고, 선택은 같은 출처의 localStorage에 저장해 storage 이벤트로 열린 탭에 반영한다. 기본은 `navigator.language`이며 미지원 언어는 영어다.
