---
title: 문서 표기
description: 문서에 쓸 수 있는 Markdown 표기와 브라우저가 그리는 범위. 표·알림·다이어그램 같은 표기를 쓸 때 읽는다.
---

지침·명세·설계 본문에서 쓰는 표기를 정한다. 브라우저가 그리는 것과 에디터·GitHub가 그리는 것이 같아야 하므로, 양쪽이 모두 아는 표기만 쓴다.
...
flowchart LR
  U[사용자] --> A[에이전트]
  A -->|파일 수정| S[".gitifact/spec · instructions"]
