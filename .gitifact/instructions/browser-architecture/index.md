...
화면을 그리는 첫 조회는 화면마다 골격을 그리지 않고 `usePageLoading`(또는 `<RequestState/>`)으로 셸에 알린다. 셸의 로더 하나가 100ms 뒤에 바로 나타나 최소 300ms 머물고, 알린 조회가 모두 답하면 본문을 한꺼번에 보인다. 화면이 보인 뒤의 조회는 같은 호출이 제자리의 작은 로더를 그린다. 화면별 골격은 두지 않는다. 큰 목록·상세는 받은 데이터를 `useDeferredValue`로 넘겨 나눠 그린다. 한 번에 그리면 그동안 로더가 멈추고, 공개 직후 프레임이 끊긴다. 첫 그리기에 쓰는 조회는 공개 뒤에 따로 나타나 본문을 밀지 않도록 모두 보고한다. 검색어 입력은 타자가 멈춘 뒤 500ms에 반영하며, `shared/lib/search`의 값(`typingDelay`) 하나를 화면 검색창과 검색창 팔레트가 함께 쓴다. 재조회 중에는 현재 목록을 유지한다.
