...
왼쪽 메뉴의 첫 항목은 제품 개요(`/product`)다. 같은 명세 응답으로 계산한 요약 타일·차트·최근 활동을 두고 머리에 ID·최근 변경·제품 문서 보기·변경 이력 링크를 곁들인다(대시보드 구성은 브라우저 명세의 설계를 따른다). PRODUCT.md의 제목·본문은 `/product/document`에서 지침 상세와 같은 읽기 페이지로 렌더링한다. 대시보드 아래에 본문을 길게 잇던 방식은 요약과 문서 읽기가 한 화면에서 서로를 밀어내 분리했다. 본문의 상대 경로 이미지는 `/api/v1/product/assets/<파일명>`으로 바꿔 서버가 제품 폴더의 이미지 파일만 이름으로 제공한다. 본문 첫 블록의 배너 이미지는 워드마크와 겹치므로 화면에서 생략한다. 지침(`/guides`)은 페이지 제목·검색·보기 전환 없이 콘텐츠 카드 전체를 열 보기로 쓴다. 폴더를 왼쪽에서 오른쪽으로 펼치고 문서를 고르면 남는 공간이 미리보기(제목·경로·ID·상세 보기 버튼·본문)가 된다. 폴더는 `folder`, 미리보기 문서는 `selected` 검색 인자에 둔다. 상세 보기는 `/guides/$id` 상세 페이지로 이동하고, 상세의 뒤로가기는 그 문서의 폴더와 미리보기로 돌아간다. 목록 보기와 검색은 2026-09-17에 사용자 요청으로 제거했다. 활동 페이지의 명세 종류 필터에 제품·지침을 더한다.
