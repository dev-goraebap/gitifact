...
| 문체는 `writing` 한 topic에 두고 spec·design·instructions 지침과 블록은 한 줄로 가리킨다 | 요구사항·설계·지침 중 무엇을 쓰든 같은 규칙 한 벌을 읽는다 | 세 문서에 나눠 적기(같은 규칙이 세 벌이 되어 고칠 때 어긋난다), 한 형식 문서의 한 절(요구사항만 쓰는 세션이 그 문서를 읽어야 전문을 본다) |
| `guide show`는 CLI가 담은 파일만 출력하고 프로젝트 파일로 바뀌는 topic을 두지 않는다 | 프로젝트마다 다른 일하는 방식은 프로젝트 지침과 AGENTS.md 색인이 담는다. CLI 출력에 프로젝트 파일을 섞으면 같은 명령이 프로젝트마다 다른 형식을 말한다 | 위키 README를 `guide show wiki` 뒤에 운영 방침으로 싣기(지침이 위키를 대신하며 없앴다), 모든 topic을 `.gitifact/overrides/<topic>.md`로 대체(`--eject`) |
