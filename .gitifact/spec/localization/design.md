...
sources:
  - title: CLI와 공통 코드
    path: ../../wiki/rules/cli.md
  - title: 브라우저
    path: ../../wiki/rules/browser.md
---
...
## 구조와 데이터
<!-- gitifact-ref: R-eeyxmpd37g, R-y6gszx7dg6 -->
...
## 처리 흐름
<!-- gitifact-ref: R-l2cviw6kk7, R-lwzfbzmgi4 -->
...
기존 블록은 --lang 명시값, 블록에 기록된 언어, 현재 CLI 언어 순으로 정한다. 언어 토큰이 없던 블록은 한국어다. 신규 블록과 최초 기본 위키만 CLI 언어로 만든다. 기존 wiki README는 docs wiki의 안내 부분과 다른 언어여도 원문을 유지한다.
...
config.language를 추가하는 이전 구상은 사용자의 브라우저별 설정 요청에 따라 채택하지 않는다. 환경마다 CLI 언어가 달라도 기존 블록 언어를 보존해 반복 업데이트 시 언어가 오가는 문제를 막는다. 공용 번역 패키지나 런타임 번역 서비스는 추가하지 않는다. 언어 선택이 기록 파일이나 JSON 계약을 바꾸지 않아 schemaVersion 2를 유지한다.
