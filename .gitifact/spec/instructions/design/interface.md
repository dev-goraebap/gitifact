...
체크아웃(browser-specs v6)은 `instructions`(지침마다 ID·폴더 이름·경로·제목·설명·본문·폴더 파일 목록·최근 변경. 파일마다 경로·크기와, Markdown이면 앞 8KB에서 코드 블록 밖의 첫 `# ` 제목 `title`)와 `agents`(루트 AGENTS.md의 본문과 최근 변경, 없거나 1MB를 넘거나 UTF-8이 아니면 `null`)를 싣는다.
