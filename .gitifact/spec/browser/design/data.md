...
| 단계 | Git 호출 |
| :--- | :--- |
| 바뀐 파일 목록 | `git log --stdin --no-walk --raw`: 묶음이 바꾼 기록 파일과 blob, 상태가 `A`인 결정기록 파일 |
| 양쪽 문서 원문 | `git cat-file --batch`: 커밋과 첫 부모 양쪽의 바뀐 문서와 그 기능의 `index.md`, 커밋이 더한 결정기록을 `<커밋>:<경로>`로 |

기록 파일은 기능 `index.md`, `requirements/*.md`, `design/*.md`, 지침 `index.md`, 결정기록 `.gitifact/records/*/*.md`, 0.8.0 이전 커밋의 위키 `*.md`와 `.gitifact/history.jsonl`이다. 기능 `index.md`는 기능 S-ID를 알려고 함께 읽으며, 그 커밋에서 바뀌지 않았으면 변경에 넣지 않는다.
...
결정기록은 한 번 쓰면 바뀌지 않으므로, 커밋의 기록은 그 커밋이 더한 기록 파일이다. 모든 기록 파일을 읽지 않고 더한 파일만 읽으며, 읽지 못하는 기록은 다른 파일처럼 빠진다. 기록은 `docs`에 적힌 문서의 변경에 붙는다.

기록 도입 전 이 저장소의 커밋은 `history.jsonl`에 더한 줄을 `git log -p -U0`의 패치로 읽어, 첫 문장(80자에서 자름)을 제목으로 하고 맥락 섹션만 있는 기록으로 붙인다. 같은 H-ID의 줄을 지우고 다시 쓴 것은 옛 이유를 고친 것이라 새 기록으로 보지 않는다. 0.7 이유도 같은 방식으로 기록이 된다.
...
모든 부모를 따라 읽어, 작업 브랜치의 원본 커밋이 자신의 첫 부모와 비교되며 작성자·시각·결정기록을 그대로 유지한다. 병합 커밋에는 병합 담당자가 실제로 손댄 문서만 남긴다.
...
브랜치에서 가져온 결정기록은 원본 커밋에 둔다. 병합에는 모든 부모 대비 새로 더한 기록 파일만 잇고, 이미 되돌린 작업의 기록이 병합에 들어와도 새 변경으로 검사하지 않는다.

> [!IMPORTANT]
> 이력을 읽으며 Git 커밋과 프로젝트 문서를 바꾸지 않는다. 재병합 비교가 실패하면 그 병합을 조용히 빠뜨리지 않고 조회 오류로 돌려준다.

`apps/cli/test/merge-history.test.mjs`가 임시 저장소에서 일반 병합, 같은 요구사항의 자동 합성, 충돌 해결, 병합 중 추가·삭제·이동, 브랜치의 자체 되돌리기, 스쿼시, 세 부모 병합, 두 브랜치에서 더한 과거 이유 줄과 결정기록, 병합이 직접 더한 결정기록, SHA-256과 캐시 재생성을 검사한다.
...
| 형식 번호(`PRAGMA user_version`, `CACHE_FORMAT`)가 다름 | 표를 지우고 다시 만든다 |
