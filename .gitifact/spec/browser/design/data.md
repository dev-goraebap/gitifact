...
`server/checkout/checkout-reader.ts`가 `.gitifact/spec`과 위키를 건드린 커밋을 `git log --name-only` 한 번으로 읽어 기능별 작성자·최근 커밋과 페이지별 최근 커밋을 센다. 로그는 커밋 20,000개까지 읽고 기능마다 2000커밋까지 센다.
