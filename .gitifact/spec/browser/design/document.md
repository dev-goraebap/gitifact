...
문서 사이의 관계는 종류마다 작은 제목 아래 세로 목록으로 둔다(`shared/ui/related-list`의 RelatedList, compact List). 한 줄에 제목이 링크, 그 아래 그 문서의 설명이다. 설계는 제목·설명 다음에 "관련 요구사항"과 "참고 문서"를 두고 선으로 본문과 나눈다. 요구사항은 본문 아래에 "이 요구사항의 설계"와 "변경 이력"을 따로 둔다. 전에는 관련 요구사항과 요구사항의 설계를 작은 글씨로 한 줄에 흘리고 이력 링크까지 같은 줄에 섞어, 무엇이 어떤 종류인지 가리기 어려웠다(2026-09-23 사용자 요청). 설계 탭은 설계 파일마다 `sources`가 있으면 본문 위에 "참고 문서" 목록을 그린다. 제목이 링크(ID로 가리킨 문서는 서버가 알려 준 경로를 같은 해석 규칙으로, url은 새 탭)이고 설명에 note와 호스트 또는 상대 경로를 둔다. 카드 대신 행으로 둔 것은 디자인 시스템 지침의 조밀한 자료 규칙을 따른 것이다.
