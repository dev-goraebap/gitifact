...
목록 항목과 표 셀의 여백·크기는 Astryx 원자 클래스가 ID 두 개 무게로 정하므로 같은 무게(`:not(#\#)` 두 번)로 덮는다. List 테마를 바꾸면 문서 이력·위키 목록까지 바뀌므로 읽기 영역 안에서만 덮는다. 인라인 코드의 띠가 회색인 것은 요구사항·설계 제목의 노란 형광펜과 구별하기 위해서다.
