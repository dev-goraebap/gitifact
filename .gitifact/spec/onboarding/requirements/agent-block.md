...
에이전트는 전역 `gitifact` 명령으로 실행한다. 프로젝트가 기준으로 삼는 CLI 버전과 블록 언어는 블록이 아니라 프로젝트 설정에 두며, 블록에는 버전을 적지 않는다. 명령이 없으면 사용자에게 설정에 적힌 버전의 전역 설치를 제안한다. 도입 프롬프트도 전역 설치 뒤 init을 실행하도록 안내한다. 프로젝트가 별도의 실행 방법을 지정하면 우선한다.
...
3. 조건: 블록이 있는 프로젝트에 참여한 팀원의 환경에 gitifact 명령이 없습니다.
   기대 동작: 에이전트는 프로젝트 설정에 적힌 버전의 전역 설치(`npm install -g gitifact@<버전>`)를 사용자에게 제안합니다. 설치나 실행 권한이 막히면 필요한 승인을 요청하고 원인을 알리며, 문서 ID 발급·검사·커밋을 대신하거나 완료했다고 보고하지 않습니다.
