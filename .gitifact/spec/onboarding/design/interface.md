...
내용은 `###` 절로 나눈 시작할 때 확인할 것(블록 버전의 전역 `gitifact`로 실행하고, 없거나 버전이 다르면 설치를 제안하며 그동안 npx로 실행하고, 권한이 막히면 필요한 승인을 요청. `instructions list --all`로 지침을 모두 읽고, 명세는 제품 동작 이야기가 나올 때와 코드를 고치기 전에 읽음), 요구사항 분류 표, 규칙, 명령 목록이다. 마커를 포함해 50줄 이내다.
