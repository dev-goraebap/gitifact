...
현재 버전은 빌드 때 주입되는 CLI 버전이며 `gitifact --version`과 같다. 에이전트가 블록에 고정된 npx 명령으로 실행하므로 비교 기준은 프로젝트의 지정 버전이 된다. CLI는 AGENTS.md를 읽어 실행 버전을 바꾸지 않는다.
...
`update --check`는 `--format text`도 지원하고, `--commit`과 함께 쓰면 Commander가 옵션 충돌로 거부한다. 동의 후에는 프로젝트의 실행 방식으로 새 CLI를 실행한다. npx는 새 버전을 직접 지정하고, 프로젝트 의존성은 기존 패키지 관리자로 갱신한다.
